...
class Entity {
  ...
  doAttack() {
    ...
    if (this.attacked.anim) {
      if (this.enemy) {
        if (this.chosenEnemy >= heroes.length)
          this.chosenEnemy = heroes.length - 1;
      }
      let enemHp;
      this.animTimeAttack.cur--;
      if (
        this.animTimeAttack.cur <
        (this.anims.attack - 1.5) * this.freq.time
      ) {
        if (this.enemy) {
          ...
          heroes[this.chosenEnemy].current.anim =
            heroes[this.chosenEnemy].anims.hit;
        } else {
          if (enemies[this.chosenEnemy]?.current) {
            ...
            enemies[this.chosenEnemy].current.img =
              enemies[this.chosenEnemy].hit;
            enemies[this.chosenEnemy].current.anim =
              enemies[this.chosenEnemy].anims.hit;
          } else {
            this.chosenEnemy--;
            if (enemies[this.chosenEnemy]?.current) {
              enemies[this.chosenEnemy].current.health -= this.damage;
      ...
      if (this.animTimeAttack.cur == 0 || enemHp <= 0) {
        if (Math.random() <= this.critChance) {
          (this.enemy ? heroes : enemies)[this.chosenEnemy].extraDamage =
            this.damaged > enemHp ? enemHp : this.damaged;
          // this.damaged+=enemHp>=this.damaged?this.damaged:enemHp;
          this.crit = true;
          if (this.enemy) waiting = true;
        }
        ...
        this.shiftCircle = 0;
        if (this.enemy) {
          heroes[this.chosenEnemy].takenDamage.cur =
            heroes[this.chosenEnemy].current.health;
          heroes[this.chosenEnemy].takenDamage.delta =
            heroes[this.chosenEnemy].takenDamage.last -
            heroes[this.chosenEnemy].takenDamage.cur;
          heroes[this.chosenEnemy].current.img = heroes[this.chosenEnemy].idle;
          heroes[this.chosenEnemy].current.anim =
            heroes[this.chosenEnemy].anims.idle;
          this.showDamaged.pos.x =
            heroes[this.chosenEnemy].pos.x +
            (heroes[this.chosenEnemy].drawnSize - 100) / 2 +
            shiftsHealth[heroes[this.chosenEnemy].name].x +
            105;
          this.showDamaged.pos.y =
            heroes[this.chosenEnemy].pos.y +
            (borders[heroes[this.chosenEnemy].name].bottom *
              heroes[this.chosenEnemy].drawnSize) /
              heroes[this.chosenEnemy].size +
            20;
          if (enemHp <= 0) heroes[this.chosenEnemy].current.animX = 0;
        } else {
          enemies[this.chosenEnemy].takenDamage.cur =
            enemies[this.chosenEnemy].current.health;
          enemies[this.chosenEnemy].takenDamage.delta =
            enemies[this.chosenEnemy].takenDamage.last -
            enemies[this.chosenEnemy].takenDamage.cur;
          enemies[this.chosenEnemy].current.img =
            enemies[this.chosenEnemy].idle;
          enemies[this.chosenEnemy].current.anim =
            enemies[this.chosenEnemy].anims.idle;
          this.showDamaged.pos.x =
            enemies[this.chosenEnemy].pos.x +
            (enemies[this.chosenEnemy].drawnSize - 100) / 2 +
            shiftsHealth[enemies[this.chosenEnemy].name].x +
            105;
          this.showDamaged.pos.y =
            enemies[this.chosenEnemy].pos.y +
            (borders[enemies[this.chosenEnemy].name].bottom *
              enemies[this.chosenEnemy].drawnSize) /
              enemies[this.chosenEnemy].size +
            20;
    ...
    if (this.attacked.from) {
      ...
      if (this.time.cur == 0) {
        ...
        if (
          enemies.length == 0 ||
          (enemies.length == 1 &&
            (enemies[0]?.current?.health <= 0 ||
              enemies[0]?.extraDamage >= enemies[0]?.current.health))
        ) {
          if (curLevel < levels.length - 1) moveAtEnd(true);
          else endGame();
        } //loadLevel(curLevel+1);
        else if (
          heroes.length == 0 ||
          (heroes.length == 1 &&
            (heroes[0]?.current?.health <= 0 ||
              heroes[0]?.extraDamage >= heroes[0]?.current.health))
        )
          endGame();
        else if (this.isBoss && !this.secondAttack) {
          this.current.img = moveAtEndParams.active ? this.move : this.idle;
          this.current.anim = moveAtEndParams.active
            ? this.anims.move
            : this.anims.idle;
          this.current.animX = 0;
          setTimeout(
          ...
          if (
            chosen == (this.enemy ? enemies.length - 1 : heroes.length - 1) &&
            !moveAtEndParams.active
          ) {
            chosen = 0;
  ...
  isHover() {
    return (
      mouse.x >=
        this.pos.x + (borders[this.name].left / this.size) * this.drawnSize &&
      mouse.x <=
        this.pos.x + (borders[this.name].right / this.size) * this.drawnSize &&
      mouse.y >= this.pos.y + 0.25 * this.drawnSize &&
      mouse.y <= this.pos.y - 0.25 * this.drawnSize + this.drawnSize
    );
  }
  hover() {
  ...
  draw(shift = 0, dt) {
    ...
    c.drawImage(
      ...
      shift +
        ...
        ((this.enemy && !this.attacked.from) ||
        (!this.enemy && this.attacked.from)
          ? this.size - borders[this.name].right + this.shiftCircle
          : borders[this.name].left - this.shiftCircle) *
          (this.drawnSize / this.size),
      this.pos.y +
        borders[this.name].bottom * (this.drawnSize / this.size) -
        this.drawnSize / 50,
      (borders[this.name].right - borders[this.name].left) *
        (this.drawnSize / this.size),
      this.drawnSize / 15,
    );
    if (this.appereance.cur > 0) {
      c.drawImage(
        Appereances[this.appereance.type].back,
        (this.appereance.anim % 4) * Appereances.sizes.width,
        Math.floor(this.appereance.anim / 4) * Appereances.sizes.height,
        Appereances.sizes.width,
        Appereances.sizes.height,
        this.pos.x,
        this.pos.y,
        this.drawnSize,
        borders[this.name].bottom * (this.drawnSize / this.size) * (400 / 263),
      );
    }
    if (this.appereance.cur < this.appereance.max * 0.6)
      c.drawImage(
        this.current.img,
        (this.enemy
          ? this.current.anim - this.current.animX - 1
          : this.current.animX) * this.size,
        0,
        this.size,
        this.size,
        this.pos.x + shift,
    ...
    if (this.chosen) {
      c.drawImage(
        ...
        this.pos.x +
          (this.enemy
            ? this.size -
              borders[this.name].left -
              (borders[this.name].right - borders[this.name].left)
            : borders[this.name].left) *
            (this.drawnSize / this.size),
        this.pos.y - 30 + borders[this.name].up * (this.drawnSize / this.size),
        (borders[this.name].right - borders[this.name].left) *
          (this.drawnSize / this.size),
        20,
      );
    ...
    if (this.appereance.cur > 0) {
      c.drawImage(
        Appereances[this.appereance.type].front,
        (this.appereance.anim % 4) * Appereances.sizes.width,
        Math.floor(this.appereance.anim / 4) * Appereances.sizes.height,
        Appereances.sizes.width,
        Appereances.sizes.height,
      ...
      if (this.appereance.time % this.appereance.timer == 0)
        this.appereance.anim++;
      if (this.appereance.anim >= 8) this.appereance.anim = 0;
      this.appereance.cur--;
    }
    if (!this.isBoss && this.appereance.cur <= 0) {
      this.drawHealth(shift);
    }
  ...
  drawHealth(shift = 0) {
    ...
    c.drawImage(
      ...
      shift +
        this.pos.x +
        (this.drawnSize - 100) / 2 +
        shiftsHealth[this.name].x,
      this.pos.y +
        (borders[this.name].bottom * this.drawnSize) / this.size +
        20,
      100,
      10,
    ); //this.drawnSize-shiftsHealth[this.name].y
    c.fillStyle = "#f5ea84";
    if (this.takenDamage.last > this.takenDamage.cur) {
      this.takenDamage.last--;
      if (
        this.takenDamage.last - this.takenDamage.cur >
          this.takenDamage.delta * 0.3 &&
        this.takenDamage.last - this.takenDamage.cur <
          this.takenDamage.delta * 0.85
      )
        this.takenDamage.last--;
      if (
        this.takenDamage.last - this.takenDamage.cur >
          this.takenDamage.delta * 0.5 &&
        this.takenDamage.last - this.takenDamage.cur <
          this.takenDamage.delta * 0.85
      )
        this.takenDamage.last--;
    ...
    c.drawImage(
      ...
      0,
      0,
      healthBar.yellow.width * (this.takenDamage.last / this.maxHp),
      healthBar.yellow.height,
      shift +
        this.pos.x +
        (this.drawnSize - 100) / 2 +
        shiftsHealth[this.name].x,
      this.pos.y +
        (borders[this.name].bottom * this.drawnSize) / this.size +
        20,
      (this.takenDamage.last / this.maxHp) * 100,
      10,
    );
    c.fillStyle = "red";
    if (this.current.health >= 0) {
      ...
      c.drawImage(
        ...
        0,
        0,
        healthBar.colour.width * (this.current.health / this.maxHp),
        healthBar.colour.height,
        shift +
          this.pos.x +
          (this.drawnSize - 100) / 2 +
          shiftsHealth[this.name].x,
        this.pos.y +
          (borders[this.name].bottom * this.drawnSize) / this.size +
          20,
        (this.current.health / this.maxHp) * 100,
        10,
      );
    }
  }
  drawHealthForBoss(shift = 0) {
    ...
    c.drawImage(
      healthBarBoss.yellow,
      0,
      0,
      healthBarBoss.yellow.width * (this.takenDamage.last / this.maxHp),
      healthBar.yellow.height,
      canvas.width * 0.2,
      canvas.height * 0.9,
      (this.takenDamage.last / this.maxHp) * wdithHealthBoss,
      heightHealthBoss,
    );
    c.fillStyle = "red";
    if (this.current.health >= 0)
      ...
      c.drawImage(
        healthBarBoss.colour,
        0,
        0,
        healthBarBoss.colour.width * (this.current.health / this.maxHp),
        healthBar.colour.height,
        canvas.width * 0.2,
        canvas.height * 0.9,
        (this.current.health / this.maxHp) * wdithHealthBoss,
        heightHealthBoss,
      );
  }
  ...
  initImages() {
    ...
    if (this.anims.countAttack > 1) {
      for (let i = 1; i <= this.anims.countAttack; i++) {
        let img =
          allImages[this.name][this.enemy ? "left" : "right"][`attack${i}`];
  ...
  attackEnemy(toPos, chosenEnemy) {
    ...
    this.speed = {
      x:
        (toPos.x +
          borders[enemies[chosenEnemy].name].left *
            (enemies[chosenEnemy].drawnSize / enemies[chosenEnemy].size) -
          (this.pos.x +
            borders[this.name].right * (this.drawnSize / this.size))) /
        this.time.max,
      y:
        (toPos.y -
          this.pos.y +
          (enemies[this.chosenEnemy].drawnSize - this.drawnSize) / 2) /
        this.time.max,
    };
    this.time.cur = this.time.max;
  ...
  attackHero() {
    ...
    this.speed = {
      x:
        (heroes[this.chosenEnemy].pos.x +
          borders[heroes[this.chosenEnemy].name].right *
            (heroes[this.chosenEnemy].drawnSize /
              heroes[this.chosenEnemy].size) -
          (this.pos.x +
            borders[this.name].left * (this.drawnSize / this.size))) /
        this.time.max,
      y:
        (heroes[this.chosenEnemy].pos.y -
          this.pos.y +
          (heroes[this.chosenEnemy].drawnSize - this.drawnSize) / 2) /
        this.time.max,
    };
...
class EntityMenu extends Entity {
  ...
  draw(dt) {
    ...
    c.drawImage(
      ...
      this.pos.x +
        (this.enemy
          ? this.size - borders[this.name].right
          : borders[this.name].left) *
          (this.drawnSize / this.size),
      this.pos.y +
        borders[this.name].bottom * (this.drawnSize / this.size) -
  ...
  showParameters() {
    if (!isMobile) {
      ...
      c.drawImage(
        ...
        this.pos.x +
          (this.enemy
            ? -150 + (1 / 4) * this.drawnSize
            : (3 / 4) * this.drawnSize),
        this.pos.y + this.drawnSize / 3 - 8,
      );
      ...
      c.fillText(
        ...
        this.pos.x +
          (this.enemy
            ...
            : (3 / 4) * this.drawnSize) +
          60,
        this.pos.y + this.drawnSize / 3 + 140,
      );
      c.drawImage(
        money,
        0,
        ...
        this.pos.x +
          (this.enemy
            ...
            : (3 / 4) * this.drawnSize) +
          10,
        this.pos.y + this.drawnSize / 3 + 130,
        30,
        30,
      );
      c.font = "30px pixel";
      ...
      c.drawImage(
        ...
        this.pos.x +
          (this.enemy
            ? -150 + (1 / 4) * this.drawnSize
            : (3 / 4) * this.drawnSize),
        this.pos.y + this.drawnSize / 3,
        50,
        50,
      );
      c.fillText(
        this.damage * (this.anims.attack - 1.5) * this.freq.time,
        this.pos.x +
          (this.enemy
            ? -150 + (1 / 4) * this.drawnSize
            : (3 / 4) * this.drawnSize) +
      ...
      c.drawImage(
        ...
        this.pos.x +
          (this.enemy
            ? -150 + (1 / 4) * this.drawnSize
            : (3 / 4) * this.drawnSize),
        this.pos.y + this.drawnSize / 3 + 60,
        50,
        50,
      );
      c.fillText(
      ...
      const topPos =
        this.pos.y + (borders[this.name].up * this.drawnSize) / this.size;
  ...
  onclick() {
    if (playerMoney >= this.cost && curHeroes.length < 5 && gameState === 0) {
      if (this.cost > 0) changeMoney(-this.cost, 1);
      playerMoney -= this.cost;
      curHeroes.push(
        generateHero(this.onpush, heroesPos[curHeroes.length + 1]),
      );
      opacity.push(1);
    }
  }
...
class Btn {
  ...
  hover() {
    ...
    this.shift =
      (this.size.height * 0.1 * this.hoverParams.value) / this.hoverParams.max;
    if (mouse.click) this.onclick();
  }
  endHover() {
    if (this.hoverParams.value > 0) {
      this.hoverParams.value--;
      this.shift =
        (this.size.height * 0.1 * this.hoverParams.value) /
        this.hoverParams.max;
    } else {
  ...
  draw() {
    ...
    if (this.isHover()) this.hover();
      ...
      if (this.hoverParams.lastState === 1 || this.hoverParams.value > 0)
        this.hoverParams.back = true;
    ...
    c.fillRect(
      ...
      this.pos.y,
      this.size.width,
      this.size.height * 0.8 + this.shift,
    );
    c.fillStyle = this.color.bottom;
    if (this.is_disabled()) c.fillStyle = "rgb(60,60,60)";
    c.fillRect(
      this.pos.x,
      this.pos.y + this.size.height * 0.8 + this.shift,
      this.size.width,
      this.size.height * 0.2 - this.shift,
    );
    ...
    c.fillText(
      ...
      this.pos.y + this.size.height * 0.5 + this.shift,
      this.size.width,
    );
  }
}
...
const money = new Image(),
  ...
  bgMenu = new Image(),
  circle = new Image(),
  caveSnowBegin = new Image(),
  grassMiddle = new Image(),
  caveSnowEnd = new Image(),
  caveSnowMiddle = new Image(),
  grassBegin = new Image(),
  grassEnd = new Image(),
  magmaMiddle = new Image(),
  healthBar = { bg: new Image(), colour: new Image(), yellow: new Image() },
  pointer = new Image(),
  custleEnd = new Image(),
  mountainPeak = new Image(),
  cemeteryMiddle = new Image(),
  ...
  shiftsHealth = {
    "girl 1": { y: 40, x: -40 },
    "evil wizard": { y: 90, x: 0 },
    "evil wizard 2": { y: 90, x: 0 },
    "medieval king": { y: 100, x: 0 },
    "Flying eye": { x: 0, y: 100 },
    Skeleton: { x: 0, y: 70 },
    Goblin: { x: 0, y: 70 },
    Mushroom: { x: 0, y: 70 },
    Striker: { x: 0, y: 0 },
  ...
  padd = 60,
  borders = {
    "Flying eye": { right: 88, left: 71, bottom: 95, up: 60 },
    "girl 1": { left: 20, right: 37, bottom: 58, up: 8 },
    "evil wizard": { right: 90, left: 61, bottom: 100, up: 44 },
    "evil wizard 2": { right: 144, left: 106, bottom: 166, up: 114 },
    Goblin: { left: 57, right: 91, bottom: 101, up: 64 },
    "medieval king": { left: 70, right: 95, bottom: 105, up: 50 },
    Mushroom: { left: 64, right: 86, bottom: 100, up: 63 },
    Skeleton: { left: 59, right: 95, bottom: 100, up: 50 },
...
money.src = `${dirImages}/money.png`;
sword.src = `${dirImages}/sword.png`;
heart.src = `${dirImages}/heart.png`;
bgParams.src = `${dirImages}/bgParams.png`;
bgMenu.src = `${dirImages}/bgmenu.png`;
circle.src = `${dirImages}/circle.png`;
caveSnowBegin.src = `${dirImages}/CaveSnow2.png`;
grassMiddle.src = `${dirImages}/grass1.png`;
grassEnd.src = `${dirImages}/grassEnd.png`;
grassBegin.src = `${dirImages}/grassBegin.png`; //CaveSnow3
caveSnowMiddle.src = `${dirImages}/CaveSnow4.png`;
caveSnowEnd.src = `${dirImages}/CaveSnow3.png`;
...
const transitionParams = {
  ...
  moveAtEndParams = {
    ...
    time: 0,
    maxTime: 100,
    maxDist: canvas.width * 0.8,
    dist: 0,
  },
  start = new Btn(
    { x: canvas.width * 0.8, y: 0 },
    { width: canvas.width * 0.2, height: canvas.height * 0.2 },
    "старт",
    { bg: "red", text: "black", bottom: "rgb(200,0,0)", hover: "rgb(220,0,0)" },
    { name: "pixel", size: canvas.height * 0.1 },
    startGame,
  ),
  firstLevels = new Btn(
    { x: canvas.width * 0.05, y: canvas.height * 0.1 },
    {
      ...
      height: (canvas.height * 0.2) / DRAWN_SIZE_RATIO,
    },
    "Лес",
    { bg: "#96A879", text: "white", bottom: "#4E553D", hover: "#5CB85C" },
    { name: "pixel", size: canvas.width * 0.05 },
    () => {
  ...
  secondLevels = new Btn(
    ...
    "Зимний лес",
    { bg: "#8BD0E5 ", text: "white", bottom: "#708090", hover: "#4DA6FF" },
    { name: "pixel", size: canvas.width * 0.05 },
    () => {
      if (!PLAYER_ACHIEVEMENTS.firstLevel.isCompleted) return;
      if (!transitionParams.active)
        transition(true, () => {
          changeMoneyParams.time = 0;
          changeMoney();
  ...
  cemeteryLevels = new Btn(
    ...
    "Мрачный город",
    { bg: "#4B0082", text: "white", bottom: "#404040", hover: "#8B0000 " },
    { name: "pixel", size: canvas.width * 0.05 },
    () => {
      if (!PLAYER_ACHIEVEMENTS.secondLevel.isCompleted) return;
      if (!transitionParams.active)
        transition(true, () => {
          changeMoneyParams.time = 0;
          changeMoney();
  ...
  thirdLevels = new Btn(
    ...
    "Подножие вуклана",
    { bg: "#D2B48C", text: "white", bottom: "#895838", hover: "#CD853F " },
    { name: "pixel", size: canvas.width * 0.05 },
    () => {
      if (!PLAYER_ACHIEVEMENTS.cemeteryLevel.isCompleted) return;
      if (!transitionParams.active)
        transition(true, () => {
          changeMoneyParams.time = 0;
          changeMoney();
  ...
  fourthLevels = new Btn(
    ...
    "Вулкан",
    { bg: "#FF4500 ", text: "white", bottom: "#8B0000", hover: "#FF5D00" },
    { name: "pixel", size: canvas.width * 0.05 },
    () => {
      if (!PLAYER_ACHIEVEMENTS.thirdLevel.isCompleted) return;
      if (!transitionParams.active)
        transition(true, () => {
          changeMoneyParams.time = 0;
          changeMoney();
  ...
  fivthLevels = new Btn(
    { x: canvas.width * 0.725, y: canvas.height * 0.1 },
    {
      width: canvas.width * 0.2,
      height: (canvas.height * 0.2) / DRAWN_SIZE_RATIO,
    },
    "Горные вершины",
    { bg: "#B0C4DE ", text: "white", bottom: "#A9A9A9", hover: "#4682B4" },
    { name: "pixel", size: canvas.width * 0.05 },
    () => {
      if (!PLAYER_ACHIEVEMENTS.fourthLevel.isCompleted) return;
      if (!transitionParams.active)
        transition(true, () => {
          changeMoneyParams.time = 0;
          changeMoney();
  ...
  enemiesPos = {
    ...
    4: { x: canvas.width * 0.55, y: canvas.height * 0.8 },
    5: { x: canvas.width * 0.9, y: canvas.height * 0.8 },
  },
  heroesPos = {
...
const // evilWizard1={damage: 2, maxHp: 200, cost: 20, countAttack: 1, critChance: 10, typeAppereance: 'purple'},
  ...
  evilWizard1Menu = new EntityMenu(
    18,
    new Entity("evil wizard 2", 250, evilWizard2),
    "evil wizard 2",
    250,
    evilWizard2,
    menuPoses[0],
  ),
  evilWizard2Menu = new EntityMenu(
    30,
    new Entity("evil wizard", 150, evilWizard1),
    "evil wizard",
    150,
    evilWizard1,
    menuPoses[2],
  ...
  medievalKingMenu = new EntityMenu(
    80,
    new Entity("medieval king", 160, medievalKing),
    "medieval king",
    160,
    medievalKing,
    menuPoses[3],
  ),
  eyeMenu = new EntityMenu(
    7,
    new Entity("Flying eye", 150, eye),
    "Flying eye",
    150,
    eye,
    menuPoses[6],
  ...
  skeletonMenu = new EntityMenu(
    20,
    new Entity("Skeleton", 150, skeleton),
    "Skeleton",
    150,
    skeleton,
    menuPoses[4],
  ...
  goblinMenu = new EntityMenu(
    2,
    new Entity("Goblin", 150, goblin),
    "Goblin",
    150,
    goblin,
    menuPoses[1],
  ),
  mushroomMenu = new EntityMenu(
    6,
    new Entity("Mushroom", 150, mushroom),
    "Mushroom",
    150,
    mushroom,
    menuPoses[5],
  ),
  warrior21Menu = new EntityMenu(
    16,
    new Entity("warrior2", 135, warrior21),
    "warrior2",
    135,
    warrior21,
    menuPoses[8],
  );
const archerMenu = new EntityMenu(
  20,
  new Entity("archer", 128, archer),
  "archer",
  128,
  archer,
  menuPoses[7],
...
var gameBgLevel,
  ...
  colorUI = "black";
var firstLevel = [
    { enemies: [{ obj: goblinObj, pos: 3 }], bg: grassBegin },
    {
      enemies: [
        { obj: goblinObj, pos: 2 },
        { obj: mushroomObj, pos: 3 },
        { obj: goblinObj, pos: 5 },
      ],
      bg: grassMiddle,
    },
  ...
  secondLevel = [
    ...
    {
      enemies: [
        { obj: warrior21Obj, pos: 4 },
        { obj: mushroomObj, pos: 5 },
        { obj: eyeObj, pos: 2 },
        { obj: mushroomObj, pos: 1 },
      ],
      bg: caveSnowMiddle,
    },
  ...
  cemeteryLevel = [
    {
      enemies: [
        { obj: goblinObj, pos: 3 },
        { obj: eyeObj, pos: 4 },
        { obj: warrior21Obj, pos: 1 },
      ],
      bg: cemeteryMiddle,
    },
    {
      enemies: [
        { obj: skeletonObj, pos: 3 },
        { obj: warrior21Obj, pos: 4 },
        { obj: mushroomObj, pos: 5 },
        { obj: archerObj, pos: 1 },
      ],
    ...
    {
      enemies: [
        { obj: skeletonObj, pos: 1 },
        { obj: archerObj, pos: 4 },
        { obj: eyeObj, pos: 3 },
        { obj: skeletonObj, pos: 5 },
      ],
      bg: cemeteryMiddle,
    },
    {
      enemies: [
        { obj: archerObj, pos: 1 },
        { obj: skeletonObj, pos: 4 },
        { obj: skeletonObj, pos: 3 },
        { obj: skeletonObj, pos: 2 },
        { obj: skeletonObj, pos: 5 },
      ],
      bg: cemeteryMiddle,
    },
  ...
  thirdLevel = [
    {
      enemies: [
        { obj: evilWizard2Obj, pos: 1 },
        { obj: evilWizard2Obj, pos: 4 },
      ],
      bg: magmaMiddle,
    },
    {
      enemies: [
        { obj: evilWizard2Obj, pos: 1 },
        { obj: evilWizard1Obj, pos: 4 },
        { obj: evilWizard2Obj, pos: 3 },
      ],
      bg: magmaMiddle,
    },
    {
      enemies: [
        { obj: evilWizard1Obj, pos: 1 },
        { obj: evilWizard1Obj, pos: 2 },
        { obj: evilWizard2Obj, pos: 4 },
        { obj: evilWizard2Obj, pos: 5 },
      ],
      bg: magmaMiddle,
    },
    {
      enemies: [
        { obj: evilWizard1Obj, pos: 3 },
        { obj: evilWizard1Obj, pos: 5 },
        { obj: evilWizard1Obj, pos: 2 },
        { obj: evilWizard2Obj, pos: 1 },
        { obj: evilWizard2Obj, pos: 4 },
      ],
...
function renderGame(deltaTime) {
  ...
  if (levels[curLevel].bg) {
    c.drawImage(
      ...
      0,
      0,
      levels[curLevel].bg.width,
      levels[curLevel].bg.height,
      0,
      0,
      canvas.width,
      canvas.height,
  ...
  c.fillRect(
    0,
    0,
    canvas.width * ((curLevel + 1) / levels.length),
    canvas.height * 0.01,
  );
  enemies.forEach((e, i) => {
    ...
    if (e.die) {
      if (
        e.isBoss ||
        (enemies.length === 1 && curLevel === levels.length - 1)
      ) {
  ...
  drawnHeroes.sort(function (a, b) {
    return (
      a.pos.y +
      (borders[a.name].up * a.drawnSize) / a.size -
      b.pos.y -
      (borders[b.name].up * b.drawnSize) / b.size
    );
  });
  drawnHeroes.forEach((h) => {
    h.draw(moveAtEndParams.dist, deltaTime);
  });
  drawnHeroes.forEach((h) => {
    if (h.isBoss) h.drawHealthForBoss();
  });
  c.font = "40px pixel";
  c.fillStyle = "white";
  c.textBaseline = "middle";
  ...
  if (changeMoneyParams.active) {
    ...
    c.fillText(
      `${changeMoneyParams.delta > 0 ? "+" : ""}${changeMoneyParams.delta}`,
      10,
      isMobile ? moneyPosY - 50 : 50,
    );
    changeMoney();
  }
  c.drawImage(
    ...
    102,
    118,
    String(playerMoney).length * 25 + 20,
    10 + moneyPosY,
    50,
    50,
  );
...
function drawHeroes() {
  for (let i = 0; i < 5; i++) {
    c.fillStyle = "black";
    c.fillRect(
      (i * canvas.width) / 10 + canvas.width / 4,
    ...
    c.fillRect(
      (i * canvas.width) / 10 +
        (canvas.width / 10 - canvas.width / 11) / 2 +
        canvas.width / 4,
      canvas.height -
        canvas.width / 10 +
        (canvas.width / 10 - canvas.width / 11) / 2,
      canvas.width / 10 - (canvas.width / 10 - canvas.width / 11),
      canvas.width / 10 - (canvas.width / 10 - canvas.width / 11),
    );
    if (i < curHeroes.length) {
      c.drawImage(
        icons[curHeroes[i].name],
        0,
        0,
        icons[curHeroes[i].name].width,
        icons[curHeroes[i].name].height,
        (i * canvas.width) / 10 +
          ((opacity[i] > 0.5 ? opacity[i] - 0.5 : 0) * canvas.width) / 10 +
          (canvas.width / 10 - canvas.width / 11) / 2 +
          canvas.width / 4,
        canvas.height -
          canvas.width / 10 +
          (canvas.width / 10 - canvas.width / 11) / 2,
        (1 - 2 * (opacity[i] > 0.5 ? opacity[i] - 0.5 : 0)) *
          (canvas.width / 11),
        canvas.width / 10 - (canvas.width / 10 - canvas.width / 11),
      );
      if (opacity[i] > 0) {
...
function renderMenu(deltaTime) {
  ...
  aviableHeroes.forEach((h) => {
    if (!h.isAvailable) c.filter = "brightness(0)";
    h.draw(deltaTime);
    c.filter = "brightness(1)";
  });
  aviableHeroes.forEach((h) => {
    if (!h.isAvailable) return;
    if (h.isHover()) h.hover();
    if (isMobile) h.showParameters();
  });
  c.font = `50px pixel`;
  c.textAlign = "left";
  c.textBaseline = "top";
...
function renderChoosingLevel() {
  ...
  aviableLevels.forEach((l, i) => {
    ...
    if (i < aviableLevels.length - 1) {
      const levelState =
        PLAYER_ACHIEVEMENTS[Object.keys(PLAYER_ACHIEVEMENTS)[i]];
      ...
      c.lineTo(
        aviableLevels[i + 1].pos.x + aviableLevels[i + 1].size.width / 2,
        aviableLevels[i + 1].pos.y + aviableLevels[i + 1].size.height / 2,
      );
      c.stroke();
      if (levelState.state > 0 && !levelState.isCompleted) {
        ...
        c.lineTo(
          l.pos.x +
            l.size.width / 2 +
            ((aviableLevels[i + 1].pos.x +
              aviableLevels[i + 1].size.width / 2 -
              l.pos.x -
              l.size.width / 2) *
              levelState.state) /
              1.4,
          l.pos.y +
            l.size.height / 2 +
            ((aviableLevels[i + 1].pos.y +
              aviableLevels[i + 1].size.height / 2 -
              l.pos.y -
              l.size.height / 2) *
              levelState.state) /
              1.4,
...
function animate() {
  ...
  if (gameState === 1) renderGame(deltaTime || 0.1);
  if (gameState === 0) {
...
function loadLevel(num) {
  ...
  enemies.forEach((h) => {
    h.current.health = h.maxHp;
    h.die = false;
    h.dieTimer = (h.anims.death - 1) * h.freq.time;
    h.current.img = h.idle;
    h.current.anim = h.anims.idle;
    h.takenDamage = { last: h.maxHp, cur: h.maxHp, delta: 0 };
  });
  ...
  if (start) {
    ...
    c.fillStyle = `rgba(0,0,0,${
      (transitionParams.to
        ...
        : transitionParams.maxTime - transitionParams.time) /
      5 /
      (transitionParams.maxTime / 5)
    })`;
    c.fillRect(0, 0, canvas.width, canvas.height);
    transitionParams.time--;
    if (transitionParams.time === 0) {
      if (!transitionParams.to) {
        transitionParams.to = true;
        transitionParams.time = transitionParams.maxTime;
...
function changeMoney(delta = 0, start = false) {
  if (start) {
    ...
    if (changeMoneyParams.time > 0) {
      ...
      changeMoneyParams.opacity =
        changeMoneyParams.time / changeMoneyParams.maxTime;
      if (
        changeMoneyParams.time > changeMoneyParams.maxTime * 0.1 &&
        changeMoneyParams.time < changeMoneyParams.maxTime * 0.5
      )
        changeMoneyParams.time--;
    } else {
      changeMoneyParams.opacity = 1;
...
function moveAtEnd(start = false) {
  if (start) {
    ...
    playerMove = false;
    heroes.forEach((h) => {
      h.current.img = h.move;
      h.current.anim = h.anims.move;
      h.current.animX = 0;
    });
  } else {
    if (moveAtEndParams.time == transitionParams.maxTime)
      transition(true, () => {
        loadLevel(curLevel + 1);
      });
...
function generateCharacterImages(name, countAttack = 1, end = "") {
  let images = {
    idle: generateImage(name, `Idle${end}`),
...
function generate(name, countAttack = 1) {
  let character = {
    right: generateCharacterImages(name, countAttack),
...
function saveData() {
  ...
  localStorage.aviableHeroes = JSON.stringify(
    aviableHeroes.filter((h) => h.isAvailable).map((h) => h.name),
  );
}
...
function loadData() {
  if (localStorage.achievements) {
    ...
    PLAYER_ACHIEVEMENTS.firstLevel = {
      ...PLAYER_ACHIEVEMENTS.firstLevel,
      ...achievements.firstLevel,
    };
    PLAYER_ACHIEVEMENTS.secondLevel = {
      ...PLAYER_ACHIEVEMENTS.secondLevel,
      ...achievements.secondLevel,
    };
    PLAYER_ACHIEVEMENTS.thirdLevel = {
      ...PLAYER_ACHIEVEMENTS.thirdLevel,
      ...achievements.thirdLevel,
    };
    PLAYER_ACHIEVEMENTS.fourthLevel = {
      ...PLAYER_ACHIEVEMENTS.fourthLevel,
      ...achievements.fourthLevel,
    };
    PLAYER_ACHIEVEMENTS.fifthLevel = {
      ...PLAYER_ACHIEVEMENTS.fifthLevel,
      ...achievements.fifthLevel,
    };
    PLAYER_ACHIEVEMENTS.cemeteryLevel = {
      ...PLAYER_ACHIEVEMENTS.cemeteryLevel,
      ...achievements.cemeteryLevel,
    };
  }
  if (localStorage.money) playerMoney = parseInt(localStorage.money);
  aviableHeroes.forEach((h) => {
    if (localStorage.aviableHeroes)
      h.isAvailable = JSON.parse(localStorage.aviableHeroes).includes(h.name);
  });
  if (playerMoney < 4) playerMoney = 4;
}
